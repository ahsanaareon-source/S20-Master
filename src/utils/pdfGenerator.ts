...
export const generateMajorWorkDetailPDF = (work: any) => {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['CDM Compliance', 'Complete', work.startDate],
...
export const generateSamplePostalPackPDFs = ({
  documentName,
  documentBody,
  ...
  recipients: Array<{ name: string; unit: string; postalAddress?: string }>;
}) => {
  const sampleRecipients = recipients.filter(recipient => recipient.postalAddress?.trim()).slice(0, 2);

  sampleRecipients.forEach((recipient, index) => {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    ...
    const mergedBody = documentBody
      .replaceAll('{{leaseholder_name}}', recipient.name)
      .replaceAll('{{leaseholder_property}}', recipient.unit)
      .replaceAll('{{postal_address}}', recipient.postalAddress || '')
      .replaceAll('{{consultation_deadline}}', consultationDeadline || '[Insert deadline date]');

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.text(documentName, marginX, cursorY);
    cursorY += 10;

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139);
    doc.text(`Sample postal pack preview ${index + 1} of ${sampleRecipients.length}`, marginX, cursorY);
    cursorY += 10;

    doc.setTextColor(15, 23, 42);
    doc.setFontSize(11);

    const paragraphs = mergedBody.split(/\n{2,}/).filter(Boolean);

    paragraphs.forEach(paragraph => {
      const lines = doc.splitTextToSize(paragraph, pageWidth - marginX * 2);
      ...
      cursorY += requiredHeight + 4;
    });

    doc.setFontSize(8);
    doc.setTextColor(100, 116, 139);
    doc.text(
      `Generated sample | ${documentName} | ${recipient.name}`,
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    );

    doc.save(`${sanitizeFilename(documentName)}-sample-${index + 1}-${sanitizeFilename(recipient.name)}.pdf`);
  });
};
