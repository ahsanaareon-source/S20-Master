...
export default function MajorWorksDetail({ work, onBack, onUpdateWork, isEditMode = false, onEditModeChange }: MajorWorksDetailProps) {
  ...
  const [comments, setComments] = useState([
    {
      ...
      comment: 'We need to expedite the NOI to all leaseholders. The consultation period is running late.',
      timestamp: '10/02/2026, 14:30',
      avatar: 'SJ',
      recipients: ['lh1', 'lh2', 'lh3', 'pm1', 'pm2']
    },
    {
      id: 2,
      author: 'Michael Thompson',
      role: 'Director',
      comment: 'Agreed. Can we also ensure that the structural survey report is shared with all directors before the next meeting?',
      timestamp: '10/02/2026, 15:45',
      avatar: 'MT',
      recipients: ['dr1', 'dr2', 'pm1', 'pm2']
    },
    {
      id: 3,
      author: 'James Cooper',
      role: 'Property Manager',
      comment: 'I\'ve uploaded the latest survey report to the documents section. Please review at your earliest convenience.',
      timestamp: '10/02/2026, 16:20',
      avatar: 'JC',
      recipients: []
    },
    {
      ...
      role: 'Contractor',
      comment: 'We can start the preliminary assessment next week. Will need access to the building on Tuesday.',
      timestamp: '09/02/2026, 11:15',
      avatar: 'ER',
      recipients: ['pm1', 'dr1', 'dr2']
    },
    {
      id: 5,
      author: 'David Chen',
      role: 'Leaseholder',
      comment: 'When can we expect to receive the cost breakdown? Several residents have been asking.',
      timestamp: '09/02/2026, 09:30',
      avatar: 'DC',
      recipients: ['pm1', 'pm2']
    }
  ]);
  ...
  const individualUsers = [
    ...
    { id: 'lh4', name: 'Sophia Turner', role: 'Leaseholder', group: 'Leaseholders', avatar: 'ST' },
    { id: 'lh5', name: 'Oliver Green', role: 'Leaseholder', group: 'Leaseholders', avatar: 'OG' },
    { id: 'lh6', name: 'Ava Patel', role: 'Leaseholder', group: 'Leaseholders', avatar: 'AP' },
    { id: 'lh7', name: 'Noah Bennett', role: 'Leaseholder', group: 'Leaseholders', avatar: 'NB' },
    { id: 'lh8', name: 'Mia Edwards', role: 'Leaseholder', group: 'Leaseholders', avatar: 'ME' },
    { id: 'lh9', name: 'Lucas Brooks', role: 'Leaseholder', group: 'Leaseholders', avatar: 'LB' },
    { id: 'lh10', name: 'Grace Murphy', role: 'Leaseholder', group: 'Leaseholders', avatar: 'GM' },
    // Directors
  ...
  const [observations, setObservations] = useState<Observation[]>([
    ...
    },
    {
      ...
      channel: 'email',
      message: 'Requested copies of the alternative contractor submissions before making further comments.',
      isObjection: false,
      status: 'addressed',
      documentId: 7,
      documentName: 'Contractor Quote - ABC Building...'
    ...
    },
    {
      ...
      channel: 'email',
      message: 'Requested a copy of the ABC insurance schedule before further comments are submitted.',
      isObjection: false,
      status: 'no-action',
      documentId: 7,
      documentName: 'Contractor Quote - ABC Building...'
...
Date of notice: [Insert date]`;
        ...
        documentsToAdd.forEach((doc: any) => {
          ...
          if (isConsultation) {
            docs.push({
              ...
              templateId: `${doc.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-standard`,
              templateName: getDefaultConsultationTemplateName(doc.name),
  ...
  const leaseholderRecords = useMemo(() => {
    ...
    const generatedLeaseholders = Array.from({
      ...
      return {
        ...
        postalAddress:
          `${flatNumber} Riverside Court, ${propertyInfo.address.split('•')[0].trim()}, London`
      };
    });
  ...
  const getInitialStages = (): Stage[] => {
    if (isNewWork && work.formData?.selectedStages && work.formData.selectedStages.length > 0) {
      ...
      return recalculateStageSequence(work.formData.selectedStages.map((stage: any) => ({
      ...
      prev.map(observation =>
        observation.id === observationId
          ? (() => {
              ...
              return {
                ...observation,
                leaseholderId: leaseholder?.id || observation.leaseholderId,
                leaseholderName: leaseholder?.name || observation.leaseholderName,
                channel: form.channel,
                message: form.message.trim(),
                isObjection: form.isObjection,
                status: form.status
    ...
    <div className="container-fluid p-4">
            ...
            {stages.map((stage) => (
              expandedStage === stage.id && (
                <div key={`tasks-${stage.id}`} className="mt-3 p-3 bg-light rounded stage-tasks-expand">
                  ...
                  {/* CDM Assessment Toggle for Tenders stage */}
                  {stage.id === 'tenders' && (
                    <div className="mt-3 pt-3 border-top">
                      <div className="d-flex align-items-center">
                        <div className="form-check form-switch d-flex align-items-center">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (isChecked) {
                                ...
                                // Clear all reasons when toggle is turned off
                                setCdmReasons({
                                  exceeds30Days: false,
                                  moreThan20Workers: false,
                                  exceeds500PersonDays: false,
      ...
      <div className="mb-4" style={{ backgroundColor: '#3b82c4', marginLeft: '-1rem', marginRight: '-1rem', paddingLeft: '1rem' }}>
        <ul className="nav mb-0" style={{ gap: '0' }}>
          ...
          <li className="nav-item">
            <button 
              className={`nav-link border-0 ${activeTab === 'comments' ? '' : ''}`}
              onClick={() => setActiveTab('comments')}
              style={{
                backgroundColor: activeTab === 'comments' ? '#ffffff' : '#3b82c4',
                color: activeTab === 'comments' ? '#000000' : '#ffffff',
                borderRadius: '0',
                padding: '0.75rem 1.5rem',
                fontWeight: '400',
                borderLeft: activeTab === 'comments' ? '3px solid #ffffff' : 'none'
              }}
            >
              Comments
            </button>
        ...
        <div>
          ...
          <div className="card border-0 shadow-sm">
            <div className="card-body p-0">
              ...
              >
                <table className="table table-hover mb-0" style={{ fontSize: '14px', minWidth: documentSegment === 'consultation' ? '1180px' : 'auto' }}>
                  <thead style={{ backgroundColor: '#f8f9fa' }}>
                    <tr>
                      ...
                      {visibleDocColumns.lastUpdated && (
                        <th 
                          className="border-0 py-3" 
                          style={{ fontWeight: '600', cursor: 'pointer', userSelect: 'none', minWidth: documentSegment === 'consultation' ? '180px' : '150px', whiteSpace: 'nowrap', paddingLeft: '0.75rem', paddingRight: '1rem' }}
                  ...
                  <tbody>
                    {paginatedDocuments.map((doc) => (
                      ...
                      >
                        ...
                        {visibleDocColumns.lastUpdated && (
                          <td className="border-0 border-bottom py-3" style={{ paddingLeft: '0.75rem', paddingRight: '1rem' }}>
                            <div style={{ whiteSpace: 'nowrap' }}>{doc.lastUpdated}</div>
                            <div className="small text-muted" style={{ whiteSpace: 'nowrap' }}>{doc.lastUpdatedBy}</div>
                          </td>
                        )}
                      </tr>
                    ))}
      ...
      {activeTab === 'comments' && (
        <div className="card border-0 shadow-sm">
          <div className="card-body">
            {/* Add Comment Form */}
            <div className="mb-4">
              <h6 className="mb-3">Add a comment</h6>
              <div className="d-flex gap-3 align-items-start">
                <div className="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center" style={{ width: '40px', height: '40px', flexShrink: 0, fontSize: '14px', fontWeight: '600' }}>
                  AJ
                </div>
                <div className="flex-grow-1">
                  {/* Recipient Selection */}
                  <div className="mb-2 position-relative">
                    <label className="form-label small text-muted mb-2">Send to (optional):</label>
                    ...
                    >
                      {selectedRecipients.length === 0 ? (
                        <span className="text-muted small">Click to select recipients (optional)...</span>
                      ) : (
                        selectedRecipients.map((recipientId, idx) => {
                          const isGroup = ['All', 'Leaseholders', 'Directors', 'Property Managers', 'Contractors'].includes(recipientId);
                          const user = individualUsers.find(u => u.id === recipientId);
                          const displayName = isGroup ? recipientId : user?.name;
                          ...
                          return (
                            <span 
                              ...
                              style={{ 
                                backgroundColor: isGroup ? '#0b81c5' : '#6c757d',
                                color: 'white',
                                fontSize: '11px',
                                fontWeight: '500',
                                padding: '4px 8px'
                            ...
                            >
                              {isGroup && <Users size={10} />}
                              {displayName}
                              <button
                                type="button"
                                className="btn-close btn-close-white"
                                style={{ fontSize: '8px', padding: 0, width: '10px', height: '10px' }}
                                onClick={(e) => {
                                  e.stopPropagation();
                    ...
                    {showRecipientDropdown && (
                      ...
                      >
                        ...
                        <div className="p-2">
                          {/* Group by role */}
                          {['Leaseholders', 'Directors', 'Property Managers', 'Contractors'].map((roleGroup) => {
                            const usersInGroup = individualUsers.filter(u => 
                              u.group === roleGroup && 
                              (u.name.toLowerCase().includes(recipientSearchQuery.toLowerCase()) ||
                               u.role.toLowerCase().includes(recipientSearchQuery.toLowerCase()))
                            );
                            
                            return usersInGroup.length > 0 && (
                              <div key={roleGroup} className="mb-3">
                                <div className="small text-muted mb-1" style={{ fontSize: '11px', fontWeight: '600' }}>
                                  {roleGroup}
                                </div>
                                      ...
                                      <input
                                        ...
                                        onChange={(e) => {
                                          const updatedRecipients = e.target.checked
                                            ? [...selectedRecipients, user.id]
                                            : selectedRecipients.filter(r => r !== user.id);
                                          setSelectedRecipients(updatedRecipients);
                                        }}
                                      />
                                      <label className="form-check-label small d-flex align-items-center gap-2" htmlFor={`user-${user.id}`} style={{ cursor: 'pointer' }}>
                                        <div className="rounded-circle bg-secondary text-white d-flex align-items-center justify-content-center" style={{ width: '20px', height: '20px', fontSize: '9px', fontWeight: '600' }}>
                                          {user.avatar}
                                        </div>
                                        <span>{user.name}</span>
                                      </label>
                          ...
                          {/* No results message */}
                          {recipientSearchQuery && 
                           !individualUsers.some(u => u.name.toLowerCase().includes(recipientSearchQuery.toLowerCase()) || u.role.toLowerCase().includes(recipientSearchQuery.toLowerCase())) && (
                            <div className="text-center py-3 text-muted">
                              <p className="small mb-0">No recipients found matching "{recipientSearchQuery}"</p>
                            </div>
                          )}
                        </div>

                        {/* Close button */}
                        <div className="p-2 border-top bg-light text-end">
                          <button 
                    ...
                    onClick={() => {
                      if (newComment.trim()) {
                        const newCommentObj = {
                          ...
                          role: 'Property Manager',
                          comment: newComment,
                          timestamp: new Date().toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
                          avatar: 'AJ',
            ...
            {/* Comments List */}
            <div className="mt-4">
              <h6 className="mb-3">Comments ({visibleComments.length})</h6>
                    ...
                    <div key={comment.id} className="d-flex gap-3">
                      ...
                      <div className="flex-grow-1">
                        <div className="d-flex align-items-center gap-2 mb-1">
                          <span className="fw-semibold" style={{ fontSize: '14px' }}>{comment.author}</span>
                          <span className="badge bg-light text-dark" style={{ fontSize: '11px', fontWeight: '500' }}>
                            {comment.role}
                          </span>
                          <span className="text-muted small ms-auto">{comment.timestamp}</span>
                          {/* Archive button for current user's comments */}
                          {comment.author === 'Ahsan Jalil' && (
                            <button
                              className="btn btn-sm btn-link text-muted p-0 ms-2"
                              onClick={() => setPendingArchiveCommentId(comment.id)}
                        ...
                        {comment.recipients && comment.recipients.length > 0 && (
                          <div className="mb-2">
                            ...
                            {comment.recipients.map((recipientId, idx) => {
                              const isGroup = ['All', 'Leaseholders', 'Directors', 'Property Managers', 'Contractors'].includes(recipientId);
                              const user = individualUsers.find(u => u.id === recipientId);
                              const displayName = isGroup ? recipientId : user?.name || recipientId;
                              
                              return (
                                <span 
                                  key={idx} 
                                  className="badge me-1 d-inline-flex align-items-center gap-1" 
                                  style={{ 
                                    backgroundColor: isGroup ? '#0b81c5' : '#6c757d',
                                    color: 'white',
                                    fontSize: '10px', 
                    ...
                    <>
                      {activityComments.map((comment, commentIndex) => (
                        <div key={comment.id} className="d-flex gap-3 mb-4 position-relative">
                          ...
                          <div className="flex-grow-1">
                            <div className="d-flex align-items-start gap-2 mb-2 flex-wrap">
                              <span className="badge" style={{ backgroundColor: '#f0fdf4', color: '#10b981', border: '1px solid #a7f3d0', fontSize: '12px', padding: '4px 10px', fontWeight: '500' }}>
                                <MessageSquare size={12} className="me-1" style={{ display: 'inline-block', verticalAlign: 'middle' }} />
                                <span style={{ verticalAlign: 'middle' }}>Comment Added</span>
                              </span>
                              {comment.archivedAt && (
                                <span className="badge bg-light text-dark border" style={{ fontSize: '12px', padding: '4px 10px', fontWeight: '500' }}>
                                  Archived
                                </span>
                              )}
                            ...
                            <div className="mb-2">
                              <div className="d-flex align-items-center gap-2 mb-1">
                                <div className="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center" style={{ width: '24px', height: '24px', fontSize: '10px', fontWeight: '600' }}>
                                  {comment.avatar}
                                </div>
                                <span style={{ fontSize: '14px' }}>{comment.author}</span>
                                <span className="badge bg-light text-dark" style={{ fontSize: '11px', fontWeight: '500' }}>
                                  {comment.role}
                                </span>
                              </div>
                              <div className="bg-light rounded p-2 mb-2" style={{ fontSize: '14px' }}>
                                {comment.comment}
                              </div>
                              {comment.recipients && comment.recipients.length > 0 && (
                                <div className="small text-muted">
                                  ...
                                  {comment.recipients.map((recipientId, idx) => {
                                    ...
                                    return (
                                      <span key={idx}>
                                        {displayName}
                                        {idx < comment.recipients.length - 1 ? ', ' : ''}
                                      </span>
                                    );
                                  })}
                                </div>
      ...
      />
      <ConfirmationModal
        show={pendingArchiveCommentId !== null}
        title="Archive comment?"
        message="This comment will be removed from active comments but kept in history."
        confirmLabel="Archive comment"
        variant="warning"
        onCancel={() => setPendingArchiveCommentId(null)}
        onConfirm={() => {
          if (pendingArchiveCommentId !== null) {
            setComments(comments.map(comment =>
              comment.id === pendingArchiveCommentId
                ? { ...comment, archivedAt: new Date().toISOString() }
                : comment
            ));
          }
          setPendingArchiveCommentId(null);
        }}
      />
      <ConfirmationModal
